...
STRONG_COLOR = hazard.DATASET_SETTINGS["strong_monsoon_start"]["color"]
WEAK_COLOR = hazard.DATASET_SETTINGS["weak_monsoon_start"]["color"]
...
def plot_data_phase_framework(write_pdf: bool = True) -> None:
    ...
    axes[0].invert_yaxis()
    axes[0].set_ylabel(r"Cheng $\delta^{18}\mathrm{O}$" "\n(per mil, inverted)")
    draw_event_rugs(axes[0], strong_ages, weak_ages, alpha=0.70, lw=0.8)
    axes[0].text(
        0.012,
        0.94,
    ...
    axes[1].invert_yaxis()
    draw_event_rugs(axes[1], strong_ages, weak_ages, alpha=0.28, lw=0.6)

    axes[2].plot(plot_grid["age_ka"], plot_grid["co2"], color=CO2_COLOR, lw=1.0)
    axes[2].set_ylabel("CO$_2$\n(ppm)")
    draw_event_rugs(axes[2], strong_ages, weak_ages, alpha=0.28, lw=0.6)
    ...
    draw_event_rugs(axes[3], strong_ages, weak_ages, alpha=0.28, lw=0.6)
    axes[3].text(
        0.012,
        0.91,
        "Phase convention: precession-index minima = 0 deg; maxima = 180 deg",
    ...
    axes[3].legend(
        handles=[
            Line2D([0], [0], color=PRE_PHASE_COLOR, lw=1.2, label="Precession phase"),
            Line2D([0], [0], color=PRE_RAW_COLOR, lw=1.2, alpha=0.45, label="Raw precession index"),
        ],
        loc="upper center",
    ...
    for ax in axes:
        ax.grid(False)
        ax.set_xlim(hazard.ANALYSIS_END_KA, hazard.ANALYSIS_START_KA)
    ...
    fig.subplots_adjust(left=0.10, right=0.90, top=0.985, bottom=0.13, hspace=0.19)
    save_figure(fig, "fig01_data_and_phase_framework", write_pdf=write_pdf)

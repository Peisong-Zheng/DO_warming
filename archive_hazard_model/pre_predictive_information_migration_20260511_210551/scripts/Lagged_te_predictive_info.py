...
from archive_hazard_model.pre_predictive_information_migration_20260511_210551.scripts.Bin_hazard_phase_poisson import (
    ANALYSIS_END_KA,
    ANALYSIS_START_KA,
    BIN_WIDTH_KA,
    DATASET_SETTINGS,
...
def add_event_history_feature(binned: pd.DataFrame) -> pd.DataFrame:
    ...
    In the Poisson models this history term is a normal fitted covariate, not
    an offset:

        log(lambda_i) = beta0 + beta_h * event_history_2ka_i + ...

    It controls for short-range clustering or inhibition in the monsoon-start
    process before asking whether a lagged climate/orbital driver adds further
    ...
    for _, group in binned.groupby("dataset_id", sort=False):
        ...
        # binned grid. The left edge uses side="right" with a tiny offset so
        # the current bin is excluded; the right edge includes bins up to
        # t + Y_HISTORY_WINDOW_KA. Thus history_i is the event count in
        # (t_i, t_i + 2 kyr], i.e. the older local past of the event process.
...
def fit_poisson(frame: pd.DataFrame, terms: tuple[str, ...]) -> dict:
    ...
    ``event_history_2ka`` in the reduced model and
    ``event_history_2ka + X_lag`` in the full model. After optimization, the
    fitted parameters are substituted back into ``poisson_loglik`` to get the
    model log likelihood used by the TE-like score.
    """
    ...
    # TE-like scan this means reduced_terms=("event_history_2ka",) and
    # full_terms=("event_history_2ka", *lagged_driver_terms).
...
def compare_nested_models(
    ...
    #   reduced: log(lambda_i) = beta0 + beta_h * event_history_2ka_i
...
def plot_single_driver_curves(single: pd.DataFrame, best_summary: pd.DataFrame, write_pdf: bool) -> None:
    ...
    for ax_idx, (ax, dataset_id) in enumerate(zip(axes, DATASET_SETTINGS)):
        ...
        ax.set_title(f"{DATASET_SETTINGS[dataset_id]['label']}: single drivers | event history", loc="left")
...
def plot_conditional_pre_phase(same_lag: pd.DataFrame, best_climate: pd.DataFrame, write_pdf: bool) -> None:
    fig, axes = plt.subplots(2, 1, figsize=(5.4, 8.8), sharex=True)
    control_styles = [
        (same_lag, "control LR04+CO2 at same lag", "#0072B2"),
        (best_climate, "control LR04+CO2 at best lags", "#D55E00"),
    ]
    for ax_idx, (ax, dataset_id) in enumerate(zip(axes, DATASET_SETTINGS)):

...
def fit_poisson_model(
    ...
    # still finite sample size relative to the number of fitted parameters. Lower AICc is better.
    aic = 2.0 * k - 2.0 * log_likelihood
    # reference for AICc: Hurvich and Tsai (1989); https://www.rdocumentation.org/packages/sme/versions/1.0.2/topics/AICc
...
def build_likelihood_tests(models: list[FittedPoissonModel]) -> pd.DataFrame:
    ...
    for dataset_id in DATASET_SETTINGS:
        for comparison_id, reduced_id, full_id in comparisons:
            ...
            # The LR=-2*(LL_reduced - LL_full), LR= 2*(LL_full - LL_reduced)
            # The ratio -2 is because -2log(L_reduced / L_full) follows chi-square distribution.
            lr_stat = 2.0 * (full.log_likelihood - reduced.log_likelihood)
            df = len(full.beta) - len(reduced.beta) # degrees of freedom, because reduced: LR04 + CO2, full:    LR04 + CO2 + sin(pre_phase) + cos(pre_phase). So df = 2

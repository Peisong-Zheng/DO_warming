...
The lagged predictive-information scan is a temporal-alignment diagnostic, not a causal-delay estimator. The best single-driver lags are reported in the main text. Their corresponding maximum gains are 0.101, 0.120, and 0.133 bits/event for LR04, \(\ce{CO2}\), and precession phase in the strong-start catalogue, and 0.113, 0.153, and 0.065 bits/event in the weak-start catalogue. The weak-start single-driver precession scan has nominal \(p=0.013\) at its best lag but is not significant after false-discovery-rate correction.

Conditional scans give the same near-zero phase result. After LR04 and \(\ce{CO2}\) are fixed at their best single-driver lags, precession phase peaks at 0 kyr for both strong starts (0.148 bits/event, \(p=8.8\times10^{-5}\)) and weak starts (0.110 bits/event, \(p=6.5\times10^{-4}\)); both remain significant after false-discovery-rate correction. When LR04, \(\ce{CO2}\), and precession phase share the same lag, the best precession lag is 0 kyr for strong starts and 0.8 kyr for weak starts. These maxima indicate contemporaneous orbital-phase organization in this binned framework, not a resolved causal response time.
...
\begin{figure}[H]
    ...
    For each row, the reduced model is specified by the ``after'' phrase: the EDC event-process baseline for the climate-state model, the Barker climate-state model for precession phase, and the EDC event-process baseline for the full predictive model.
    }

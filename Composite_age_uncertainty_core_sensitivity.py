...
def best_match_uncertainty_source_path() -> Path:
    return (
        ...
        else BEST_MATCH_UNCERTAINTY_FALLBACK_CSV
    )
...
def load_best_match_uncertainty() -> pd.DataFrame:
    ...
    if not source_path.exists():
        raise FileNotFoundError(
            "Cannot find the best-match uncertainty table at either "
            f"{BEST_MATCH_UNCERTAINTY_CSV} or {BEST_MATCH_UNCERTAINTY_FALLBACK_CSV}."
        )
    raw = pd.read_csv(source_path)
    age_source_col = "composite_age_ka" if "composite_age_ka" in raw.columns else "age_ka"
    uncertainty_source_col = (
        "age_uncertainty_2sigma_ka"
        if "age_uncertainty_2sigma_ka" in raw.columns
        else "age_uncertainty_2sigma_kyr"
    )
    out = pd.DataFrame(
        {
            "age_ka": pd.to_numeric(raw[age_source_col], errors="coerce"),
            "age_uncertainty_2sigma_kyr": pd.to_numeric(
                raw[uncertainty_source_col],
                errors="coerce",
            ),
    ...
    out["source"] = relative_path(source_path)
...
def build_hazard_base() -> tuple[pd.DataFrame, np.ndarray]:
    ...
    Only the event ages change between Monte Carlo realizations. The forcings,
    precession phase convention, bin edges, and Cheng sampling-resolution
    control are fixed on the common 0.2 kyr grid and can therefore be computed
    once.
    """
...
def full_hazard_metrics(
    ...
    *,
    ...
    n_hazard_events = int(fit_frame["event_count"].sum())
    return {
...
def main() -> None:
    ...
    parameters = {
        ...
        "best_match_uncertainty_csv": relative_path(best_match_uncertainty_source_path()),
